...
for I in `($AWS ec2 describe-instances --query "Reservations[*].Instances[*].InstanceId" || console_output "ERROR" "Failed to list instances") | uniq`;
do
    echo "Terminate ${I}"
...
for SN in `($AWS ec2 describe-subnets --query "Subnets[*].SubnetId" || console_output "ERROR" "Failed to list subnets") | uniq`;
do
    echo "Delete ${SN}"
...
for V in `($AWS ec2 describe-vpcs --query "Vpcs[*].VpcId" || console_output "ERROR" "Failed to list VPCs") | uniq`;
do
    echo "Delete ${V}"
...
for RT in `($AWS ec2 describe-route-tables --query "RouteTables[*].RouteTableId" || console_output "ERROR" "Failed to list VPCs") | uniq`;
do
    echo "Delete ${RT}"
